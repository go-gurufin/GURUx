...
func DeployContract(
	...
	chainID := guruxApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := guruxApp.EvmKeeper.GetNonce(ctx, from)
	...
	msgEthereumTx := evm.NewTx(&evm.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: guruxApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := DeliverEthTx(guruxApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, guruxApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := guruxApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := guruxApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := guruxApp.EvmKeeper.GetNonce(ctx, from)
