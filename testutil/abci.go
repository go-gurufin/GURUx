...
import (
	...
	"github.com/gurufin2021/GURUx/testutil/tx"
)
...
//  3. EndBlock
//  4. Commit
func Commit(ctx sdk.Context, app *app.Gurux, t time.Duration, vs *tmtypes.ValidatorSet) (sdk.Context, error) {
...
func DeliverTx(
	...
	return BroadcastTxBytes(appGurux, txConfig.TxEncoder(), tx)
}
...
func DeliverEthTx(
	...
	tx, err := tx.PrepareEthTx(txConfig, appGurux, priv, msgs...)
	if err != nil {
		return abci.ResponseDeliverTx{}, err
	}
	return BroadcastTxBytes(appGurux, txConfig.TxEncoder(), tx)
...
func CheckTx(
	...
	return checkTxBytes(appGurux, txConfig.TxEncoder(), tx)
}
...
func BroadcastTxBytes(app *app.Gurux, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseDeliverTx, error) {
...
func checkTxBytes(app *app.Gurux, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseCheckTx, error) {
	bz, err := txEncoder(tx)
	if err != nil {
		return abci.ResponseCheckTx{}, err
	}
