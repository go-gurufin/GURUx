...
func PrepareEIP712CosmosTx(
	...
	accNumber := appGurux.AccountKeeper.GetAccount(ctx, from).GetAccountNumber()

	nonce, err := appGurux.AccountKeeper.GetSequence(ctx, from)
	if err != nil {
		return nil, err
	}
...
func createTypedData(args typedDataArgs, useLegacy bool) (apitypes.TypedData, error) {
	if useLegacy {
		...
		guruxCodec := codec.NewProtoCodec(registry)
