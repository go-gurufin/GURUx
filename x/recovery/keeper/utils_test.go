...
func (suite *IBCTestingSuite) SetupTest() {
	...
	err := suite.GuruxChain.App.(*app.Gurux).BankKeeper.MintCoins(suite.GuruxChain.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)

	// Fund sender address to pay fees
	err = suite.GuruxChain.App.(*app.Gurux).BankKeeper.SendCoinsFromModuleToAccount(suite.GuruxChain.GetContext(), inflationtypes.ModuleName, suite.GuruxChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)

	coinGurux = sdk.NewCoin(utils.BaseDenom, sdk.NewInt(10000))
	coins = sdk.NewCoins(coinGurux)
	err = suite.GuruxChain.App.(*app.Gurux).BankKeeper.MintCoins(suite.GuruxChain.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = suite.GuruxChain.App.(*app.Gurux).BankKeeper.SendCoinsFromModuleToAccount(suite.GuruxChain.GetContext(), inflationtypes.ModuleName, suite.IBCOsmosisChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	err = suite.GuruxChain.App.(*app.Gurux).ClaimsKeeper.SetParams(suite.GuruxChain.GetContext(), claimparams)
	suite.Require().NoError(err)

	params := types.DefaultParams()
	params.EnableRecovery = true
	err = suite.GuruxChain.App.(*app.Gurux).RecoveryKeeper.SetParams(suite.GuruxChain.GetContext(), params)
	suite.Require().NoError(err)

	evmParams := suite.GuruxChain.App.(*app.Gurux).EvmKeeper.GetParams(s.GuruxChain.GetContext())
	evmParams.EvmDenom = utils.BaseDenom
	err = suite.GuruxChain.App.(*app.Gurux).EvmKeeper.SetParams(s.GuruxChain.GetContext(), evmParams)
	suite.Require().NoError(err)
