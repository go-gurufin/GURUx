...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	suite.GuruxChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(1))
	suite.IBCOsmosisChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(2))
	suite.IBCCosmosChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(3))
	suite.coordinator.CommitNBlocks(suite.GuruxChain, 2)
	suite.coordinator.CommitNBlocks(suite.IBCOsmosisChain, 2)
	suite.coordinator.CommitNBlocks(suite.IBCCosmosChain, 2)

	s.app = suite.GuruxChain.App.(*app.Gurux)
	evmParams := s.app.EvmKeeper.GetParams(s.GuruxChain.GetContext())
	evmParams.EvmDenom = utils.BaseDenom
	err := s.app.EvmKeeper.SetParams(s.GuruxChain.GetContext(), evmParams)
	suite.Require().NoError(err)

	// s.app.FeeMarketKeeper.SetBaseFee(s.GuruxChain.GetContext(), big.NewInt(1))
	...
	validators := s.app.StakingKeeper.GetValidators(suite.GuruxChain.GetContext(), 2)
	cons, err := validators[0].GetConsAddr()
	suite.Require().NoError(err)
	suite.GuruxChain.CurrentHeader.ProposerAddress = cons.Bytes()

	err = s.app.StakingKeeper.SetValidatorByConsAddr(suite.GuruxChain.GetContext(), validators[0])
	suite.Require().NoError(err)

	_, err = s.app.EvmKeeper.GetCoinbaseAddress(suite.GuruxChain.GetContext(), sdk.ConsAddress(suite.GuruxChain.CurrentHeader.ProposerAddress))
	suite.Require().NoError(err)
	// Mint coins locked on the gurux account generated with secp.
	...
	err = s.app.BankKeeper.MintCoins(suite.GuruxChain.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToAccount(suite.GuruxChain.GetContext(), inflationtypes.ModuleName, suite.GuruxChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(s.GuruxChain.GetContext(), types.ModuleName, coins)
	s.Require().NoError(err)
	coins = sdk.NewCoins(sdk.NewCoin(teststypes.UatomIbcdenom, sdk.NewInt(100)))
	err = s.app.BankKeeper.MintCoins(s.GuruxChain.GetContext(), types.ModuleName, coins)
	s.Require().NoError(err)

	// Mint coins on the osmosis side which we'll use to unlock our agurux
	...
	// Mint coins on the cosmos side which we'll use to unlock our agurux
	...
	claimparams.AirdropStartTime = suite.GuruxChain.GetContext().BlockTime()
	claimparams.EnableClaims = true
	err = s.app.ClaimsKeeper.SetParams(suite.GuruxChain.GetContext(), claimparams)
	suite.Require().NoError(err)

	params := types.DefaultParams()
	params.EnableErc20 = true
	err = s.app.Erc20Keeper.SetParams(suite.GuruxChain.GetContext(), params)
	suite.Require().NoError(err)

	suite.pathOsmosisGurux = ibctesting.NewTransferPath(suite.IBCOsmosisChain, suite.GuruxChain) // clientID, connectionID, channelID empty
	suite.pathCosmosGurux = ibctesting.NewTransferPath(suite.IBCCosmosChain, suite.GuruxChain)
	suite.pathOsmosisCosmos = ibctesting.NewTransferPath(suite.IBCCosmosChain, suite.IBCOsmosisChain)
	ibctesting.SetupPath(suite.coordinator, suite.pathOsmosisGurux) // clientID, connectionID, channelID filled
	ibctesting.SetupPath(suite.coordinator, suite.pathCosmosGurux)
	ibctesting.SetupPath(suite.coordinator, suite.pathOsmosisCosmos)
	suite.Require().Equal("07-tendermint-0", suite.pathOsmosisGurux.EndpointA.ClientID)
	suite.Require().Equal("connection-0", suite.pathOsmosisGurux.EndpointA.ConnectionID)
	suite.Require().Equal("channel-0", suite.pathOsmosisGurux.EndpointA.ChannelID)

	coinGurux = sdk.NewCoin(utils.BaseDenom, sdk.NewInt(1000000000000000000))
	coins = sdk.NewCoins(coinGurux)
	err = s.app.BankKeeper.MintCoins(suite.GuruxChain.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(suite.GuruxChain.GetContext(), types.ModuleName, authtypes.FeeCollectorName, coins)
	suite.Require().NoError(err)
}
...
// to the Gurux chain (used on IBC tests)
func (suite *KeeperTestSuite) DeployContractToChain(name, symbol string, decimals uint8) (common.Address, error) {
	return testutil.DeployContract(
		s.GuruxChain.GetContext(),
		s.GuruxChain.App.(*app.Gurux),
		suite.GuruxChain.SenderPrivKey,
		suite.queryClientEvm,
		contracts.ERC20MinterBurnerDecimalsContract,
		name, symbol, decimals,
	)
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/agurux") as input parameter.
