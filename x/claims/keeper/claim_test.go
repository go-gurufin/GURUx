...
func (suite *KeeperTestSuite) TestDelegationAutoWithdrawAndDelegateMore() {
	...
	validator, _ = validator.AddTokensFromDel(sdk.TokensFromConsensusPower(1, gurutypes.PowerReduction))
	delAmount := sdk.TokensFromConsensusPower(1, gurutypes.PowerReduction)
...
func (suite *KeeperTestSuite) TestClawbackEmptyAccountsAirdrop() {
	...
	tests := []struct {
		...
		{
			name:           "address active",
			address:        "gurux1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			sequence:       1,
		...
		},
		{
			...
			address:        "gurux1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
			sequence:       0,
			expectClawback: true,
			claimsRecord: types.ClaimsRecord{
				InitialClaimableAmount: sdk.NewInt(100),
