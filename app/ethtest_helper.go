...
// EthSetup initializes a new GuruxApp. A Nop logger is set in GuruxApp.
func EthSetup(isCheckTx bool, patchGenesis func(*Gurux, simapp.GenesisState) simapp.GenesisState) *Gurux {
	return EthSetupWithDB(isCheckTx, patchGenesis, dbm.NewMemDB())
}

// EthSetupWithDB initializes a new GuruxApp. A Nop logger is set in GuruxApp.
func EthSetupWithDB(isCheckTx bool, patchGenesis func(*Gurux, simapp.GenesisState) simapp.GenesisState, db dbm.DB) *Gurux {
	app := NewGurux(log.NewNopLogger(),
		db,
		nil,
		true,
		map[int64]bool{},
