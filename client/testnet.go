...
import (
	...
	"github.com/gurufin2021/GURUx/testutil/network"
)
...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(sdkserver.FlagMinGasPrices, fmt.Sprintf("0.000006%s", gurutypes.AttoGurux),
...
Example:
	guruxd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
...
Example:
	guruxd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, gurutypes.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(gurutypes.AttoGurux, accStakingTokens),
		}
		...
		valTokens := sdk.TokensFromConsensusPower(100, gurutypes.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(gurutypes.AttoGurux, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(gurutypes.AttoGurux)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, gurutypes.AttoGurux, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
