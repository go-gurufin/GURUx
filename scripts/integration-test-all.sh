...
## default port prefixes for guruxd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
DATA_DIR=$(mktemp -d -t gurux-datadir.XXXXX)
...
init_func() {
    "$PWD"/build/guruxd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/guruxd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/guruxd add-genesis-account \
    "$("$PWD"/build/guruxd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000agurux,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/guruxd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/guruxd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/guruxd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/guruxd start --pruning=nothing --rpc.unsafe \
    ...
    GURUX_PID=$!
    echo "started gurux node, pid=$GURUX_PID"
    # add PID to array
    arr+=("$GURUX_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test gurux node $HOST_RPC ..."
...
stop_func() {
    GURUX_PID=$i
    echo "shutting down node, pid=$GURUX_PID ..."
