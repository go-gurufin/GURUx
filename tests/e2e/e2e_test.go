...
// TestUpgrade tests if an Gurux node can be upgraded from one version to another.
...
// TestCLITxs executes different types of transactions against an Gurux node
	...
	}{
		{
			...
			cmd: func() (string, error) {
				return s.upgradeManager.CreateSubmitProposalExec(
					...
					5000,
					true,
					"--fees=5000000000agurux",
					"--gas-prices=50000agurux",
				)
			},
			expPass:   false,
